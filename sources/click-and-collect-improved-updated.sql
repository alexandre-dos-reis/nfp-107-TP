...
CREATE DEFINER=`root`@`localhost` FUNCTION `getFreeEmployee`(`id_timeslot` INT) RETURNS int
    NO SQL
BEGIN
DECLARE res INT;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `isTimeslotFull`(`id_timeslot` INT) RETURNS int
    NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateOrderAmount`(IN `id_Order` INT)
BEGIN
...
CREATE TABLE `order` (
  ...
  CONSTRAINT `order_ibfk_1` FOREIGN KEY (`idUser`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `order_ibfk_2` FOREIGN KEY (`idEmployee`) REFERENCES `employee` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `order_ibfk_3` FOREIGN KEY (`idTimeslot`) REFERENCES `timeslot` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
...
(3,	'2021-03-03 18:44:16',	1,	2,	0,	14774,	200,	1,	1,	1),
...
CREATE TRIGGER `before_insert_order` BEFORE INSERT ON `order` FOR EACH ROW
BEGIN
...
CREATE TRIGGER `after_insert_order` AFTER INSERT ON `order` FOR EACH ROW
if (NEW.idTimeslot is NOT NULL) THEN
    call updateTimeSlot(NEW.idTimeslot);
END IF;;

CREATE TRIGGER `update_order` AFTER UPDATE ON `order` FOR EACH ROW
if (NEW.idTimeslot is NOT NULL) THEN
    call updateTimeSlot(NEW.idTimeslot);
...
CREATE TRIGGER `delete_order` AFTER DELETE ON `order` FOR EACH ROW
...
CREATE TABLE `orderdetail` (
  ...
  KEY `idProduct` (`idProduct`),
  CONSTRAINT `orderdetail_ibfk_1` FOREIGN KEY (`idOrder`) REFERENCES `order` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `orderdetail_ibfk_2` FOREIGN KEY (`idProduct`) REFERENCES `product` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TRIGGER `insert_order_detail` AFTER INSERT ON `orderdetail` FOR EACH ROW
CALL updateOrderAmount (
        NEW.idOrder
    );;

CREATE TRIGGER `update_order_detail` AFTER UPDATE ON `orderdetail` FOR EACH ROW
CALL updateOrderAmount (
        NEW.idOrder
    );;

CREATE TRIGGER `delete_order_detail` AFTER DELETE ON `orderdetail` FOR EACH ROW
CALL updateOrderAmount (
        OLD.idOrder
    );;
...
(6,	'1962 LanciaA Delta 16V',	'Features include: Turnable front wheels; steering function; detailed interior; detailed engine; opening hood; opening trunk; opening doors; and detailed chassis.',	6791,	'S10_4962',	14774,	0,	1),
...
(18,	'1993 Mazda RX-7',	'This model features, opening hood, opening doors, detailed engine, rear spoiler, opening trunk, working steering, tinted windows, baked enamel finish. Color red.',	3975,	'S18_1129',	14154,	0,	1),
