...
DROP DATABASE IF EXISTS `click-and-collect-ubiquity`;
CREATE DATABASE `click-and-collect-ubiquity` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `click-and-collect-ubiquity`;

DELIMITER ;;

CREATE FUNCTION `getFreeEmployee`(`id_timeslot` int) RETURNS int
    NO SQL
BEGIN
DECLARE res INT;

SET res=(SELECT e.id FROM employee e where e.id not in (select o.idEmployee from `purchase` o inner join timeslot t on o.idTimeslot=t.id where t.id=id_timeslot and o.idEmployee is not null) limit 1);
RETURN res;
END;;

CREATE FUNCTION `getPackPromo`(`id_pack` int) RETURNS int
    NO SQL
BEGIN
DECLARE old_p int;
DECLARE new_p int;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `isTimeslotExpired`(`id_timeslot` INT) RETURNS tinyint(1)
    NO SQL
return (select (slotDate>=CURDATE()-0.5) from timeslot WHERE id=id_timeslot);;

CREATE FUNCTION `isTimeslotFull`(`id_timeslot` int) RETURNS int
    NO SQL
return (SELECT count(*) FROM `purchase` WHERE idTimeslot=id_timeslot AND idEmployee is NULL)>=(SELECT COUNT(*) FROM employee e where e.id not in (select o.idEmployee from `purchase` o inner join timeslot t on o.idTimeslot=t.id where t.id=id_timeslot and o.idEmployee is not null));;

CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_slot`(IN `d` DATE)
    NO SQL
BEGIN
INSERT INTO timeslot (`slotDate`, `full`, `expired`) VALUES (@d, '0', '0');
END;;

CREATE DEFINER=`root`@`localhost` PROCEDURE `timeslot_generation`(IN `date_limit` DATE)
    NO SQL
BEGIN
...
CREATE PROCEDURE `updateOrderAmount`(IN `id_Order` int)
BEGIN
	update `purchase` set amount=(select sum(p.price*o.quantity) from purchasedetail o inner JOIN product p on p.id=o.idProduct where o.idOrder=id_Order) where id=id_Order;
    	update `purchase` set toPay=(select sum(p.price*o.quantity) from purchasedetail o inner JOIN product p on p.id=o.idProduct where o.idOrder=id_Order and o.prepared) where id=id_Order;
        update `purchase` set missingNumber=(select sum(o.quantity) from purchasedetail o inner JOIN product p on p.id=o.idProduct where o.idOrder=id_Order and !o.prepared) where id=id_Order;
        update `purchase` set itemsNumber=(select sum(o.quantity) from purchasedetail o inner JOIN product p on p.id=o.idProduct where o.idOrder=id_Order) where id=id_Order;
END;;

CREATE DEFINER=`root`@`localhost` PROCEDURE `updateTimeSlot`(IN `id_timeslot` INT)
    NO SQL
...
CREATE TABLE `associatedproduct` (
  ...
  `idAssoProduct` int NOT NULL,
  PRIMARY KEY (`idProduct`,`idAssoProduct`),
  KEY `productsasso_ibfk_1` (`idAssoProduct`),
  CONSTRAINT `associatedproduct_ibfk_1` FOREIGN KEY (`idAssoProduct`) REFERENCES `product` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `associatedproduct_ibfk_2` FOREIGN KEY (`idProduct`) REFERENCES `product` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

TRUNCATE `associatedproduct`;
...
CREATE TABLE `basket` (
  ...
  PRIMARY KEY (`id`),
  KEY `idUser` (`idUser`),
  CONSTRAINT `basket_ibfk_1` FOREIGN KEY (`idUser`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `basketdetail` (
  ...
  CONSTRAINT `basketdetail_ibfk_1` FOREIGN KEY (`idBasket`) REFERENCES `basket` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `basketdetail_ibfk_2` FOREIGN KEY (`idProduct`) REFERENCES `product` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

TRUNCATE `basketdetail`;
...
CREATE TABLE `pack` (
  `idProduct` int NOT NULL,
  `idPack` int NOT NULL,
  PRIMARY KEY (`idProduct`,`idPack`),
  KEY `idProduct` (`idPack`),
  CONSTRAINT `pack_ibfk_1` FOREIGN KEY (`idProduct`) REFERENCES `product` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `pack_ibfk_2` FOREIGN KEY (`idPack`) REFERENCES `product` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TRIGGER `insert_associated` AFTER INSERT ON `pack` FOR EACH ROW
BEGIN
DECLARE promo int;
...
CREATE TRIGGER `delete_associated` AFTER DELETE ON `pack` FOR EACH ROW
BEGIN
...
CREATE TABLE `product` (
  ...
  `promotion` mediumint NOT NULL,
  `idSection` int NOT NULL,
  PRIMARY KEY (`id`),
  KEY `idSection` (`idSection`),
  FULLTEXT KEY `name` (`name`),
  FULLTEXT KEY `comments` (`comments`),
  CONSTRAINT `product_ibfk_1` FOREIGN KEY (`idSection`) REFERENCES `section` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=154 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

TRUNCATE `product`;
INSERT INTO `product` (`id`, `name`, `comments`, `stock`, `image`, `price`, `promotion`, `idSection`) VALUES
(4,	'2003 Harley-Davidson Eagle Drag Bike',	'Model features, official Harley Davidson logos and insignias, detachable rear wheelie bar, heavy diecast metal with resin parts, authentic multi-color tampo-printed graphics, separate engine drive belts, free-turning front fork, rotating tires and rear racing slick, certificate of authenticity, detailed engine, display stand\r\n, precision diecast replica, baked enamel finish, 1:10 scale model, removable fender, seat and tank cover piece for displaying the superior detail of the v-twin engine',	5582,	'S10_4698',	19366,	0,	2),
(5,	'1972 Alfa Romeo GTA',	'Features include: Turnable front wheels; steering function; detailed interior; detailed engine; opening hood; opening trunk; opening doors; and detailed chassis.',	3252,	'S10_4757',	13600,	0,	1),
(6,	'1962 LanciaA Delta 16V',	'Features include: Turnable front wheels; steering function; detailed interior; detailed engine; opening hood; opening trunk; opening doors; and detailed chassis.',	6688,	'S10_4962',	14774,	0,	1),
(8,	'2001 Ferrari Enzo',	'Turnable front wheels; steering function; detailed interior; detailed engine; opening hood; opening trunk; opening doors; and detailed chassis.',	3619,	'S12_1108',	20780,	0,	1),
(10,	'2002 Suzuki XREO',	'Official logos and insignias, saddle bags located on side of motorcycle, detailed engine, working steering, working suspension, two leather seats, luggage rack, dual exhaust pipes, small saddle bag located on handle bars, two-tone paint with chrome accents, superior die-cast detail , rotating wheels , working kick stand, diecast metal with plastic parts and baked enamel finish.',	9997,	'S12_2823',	15062,	0,	2),
(11,	'1969 Corvair Monza',	'1:18 scale die-cast about 10\" long doors open, hood opens, trunk opens and wheels roll',	6906,	'S12_3148',	15108,	0,	1),
(12,	'1968 Dodge Charger',	'1:12 scale model of a 1968 Dodge Charger. Hood, doors and trunk all open to reveal highly detailed interior features. Steering wheel actually turns the front wheels. Color black',	9123,	'S12_3380',	11744,	0,	1),
(13,	'1969 Ford Falcon',	'Turnable front wheels; steering function; detailed interior; detailed engine; opening hood; opening trunk; opening doors; and detailed chassis.',	1049,	'S12_3891',	17302,	0,	1),
(14,	'1970 Plymouth Hemi Cuda',	'Very detailed 1970 Plymouth Cuda model in 1:12 scale. The Cuda is generally accepted as one of the fastest original muscle cars from the 1970s. This model is a reproduction of one of the orginal 652 cars built in 1970. Red color.',	5663,	'S12_3990',	7980,	0,	1),
(15,	'1957 Chevy Pickup',	'1:12 scale die-cast about 20\" long Hood opens, Rubber wheels',	6125,	'S12_4473',	11850,	0,	6),
(16,	'1969 Dodge Charger',	'Detailed model of the 1969 Dodge Charger. This model includes finely detailed interior and exterior features. Painted in red and white.',	7323,	'S12_4675',	11516,	0,	1),
(17,	'1940 Ford Pickup Truck',	'This model features soft rubber tires, working steering, rubber mud guards, authentic Ford logos, detailed undercarriage, opening doors and hood,  removable split rear gate, full size spare mounted in bed, detailed interior with opening glove box',	2613,	'S18_1097',	11667,	0,	6),
(18,	'1993 Mazda RX-7',	'This model features, opening hood, opening doors, detailed engine, rear spoiler, opening trunk, working steering, tinted windows, baked enamel finish. Color red.',	3966,	'S18_1129',	14154,	0,	1),
(19,	'1937 Lincoln Berline',	'Features opening engine cover, doors, trunk, and fuel filler cap. Color black',	8693,	'S18_1342',	10274,	0,	7),
(20,	'1936 Mercedes-Benz 500K Special Roadster',	'This 1:18 scale replica is constructed of heavy die-cast metal and has all the features of the original: working doors and rumble seat, independent spring suspension, detailed interior, working steering system, and a bifold hood that reveals an engine so accurate that it even includes the wiring. All this is topped off with a baked enamel finish. Color white.',	8635,	'S18_1367',	5391,	0,	7),
(21,	'1965 Aston Martin DB5',	'Die-cast model of the silver 1965 Aston Martin DB5 in silver. This model includes full wire wheels and doors that open with fully detailed passenger compartment. In 1:18 scale, this model measures approximately 10 inches/20 cm long.',	9042,	'S18_1589',	12444,	0,	1),
(23,	'1917 Grand Touring Sedan',	'This 1:18 scale replica of the 1917 Grand Touring car has all the features you would expect from museum quality reproductions: all four doors and bi-fold hood opening, detailed engine and instrument panel, chrome-look trim, and tufted upholstery, all topped off with a factory baked-enamel finish.',	2724,	'S18_1749',	17000,	0,	7),
(24,	'1948 Porsche 356-A Roadster',	'This precision die-cast replica features opening doors, superb detail and craftsmanship, working steering system, opening forward compartment, opening rear trunk with removable spare, 4 wheel independent spring suspension as well as factory baked enamel finish.',	8824,	'S18_1889',	7700,	0,	1),
(25,	'1995 Honda Civic',	'This model features, opening hood, opening doors, detailed engine, rear spoiler, opening trunk, working steering, tinted windows, baked enamel finish. Color yellow.',	9772,	'S18_1984',	14225,	0,	1),
(26,	'1998 Chrysler Plymouth Prowler',	'Turnable front wheels; steering function; detailed interior; detailed engine; opening hood; opening trunk; opening doors; and detailed chassis.',	4724,	'S18_2238',	16373,	0,	1),
(27,	'1911 Ford Town Car',	'Features opening hood, opening doors, opening trunk, wide white wall tires, front door arm rests, working steering system.',	540,	'S18_2248',	6054,	0,	7),
(28,	'1964 Mercedes Tour Bus',	'Exact replica. 100+ parts. working steering system, original logos',	8258,	'S18_2319',	12273,	0,	6),
(29,	'1932 Model A Ford J-Coupe',	'This model features grille-mounted chrome horn, lift-up louvered hood, fold-down rumble seat, working steering system, chrome-covered spare, opening doors, detailed and wired engine',	9354,	'S18_2325',	12713,	0,	7),
(30,	'1926 Ford Fire Engine',	'Gleaming red handsome appearance. Everything is here the fire hoses, ladder, axes, bells, lanterns, ready to fight any inferno.',	2018,	'S18_2432',	6077,	0,	6),
(32,	'1936 Harley Davidson El Knucklehead',	'Intricately detailed with chrome accents and trim, official die-struck logos and baked enamel finish.',	4357,	'S18_2625',	6057,	0,	2),
(33,	'1928 Mercedes-Benz SSK',	'This 1:18 replica features grille-mounted chrome horn, lift-up louvered hood, fold-down rumble seat, working steering system, chrome-covered spare, opening doors, detailed and wired engine. Color black.',	548,	'S18_2795',	16875,	0,	7),
(34,	'1999 Indy 500 Monte Carlo SS',	'Features include opening and closing doors. Color: Red',	8164,	'S18_2870',	13200,	0,	1),
(35,	'1913 Ford Model T Speedster',	'This 250 part reproduction includes moving handbrakes, clutch, throttle and foot pedals, squeezable horn, detailed wired engine, removable water, gas, and oil cans, pivoting monocle windshield, all topped with a baked enamel red finish. Each replica comes with an Owners Title and Certificate of Authenticity. Color red.',	4189,	'S18_2949',	10131,	0,	7),
(36,	'1934 Ford V8 Coupe',	'Chrome Trim, Chrome Grille, Opening Hood, Opening Doors, Opening Trunk, Detailed Engine, Working Steering System',	5649,	'S18_2957',	6246,	0,	7),
(37,	'1999 Yamaha Speed Boat',	'Exact replica. Wood and Metal. Many extras including rigging, long boats, pilot house, anchors, etc. Comes with three masts, all square-rigged.',	4259,	'S18_3029',	8602,	0,	4),
(38,	'18th Century Vintage Horse Carriage',	'Hand crafted diecast-like metal horse carriage is re-created in about 1:18 scale of antique horse carriage. This antique style metal Stagecoach is all hand-assembled with many different parts.\r\n\r\nThis collectible metal horse carriage is painted in classic Red, and features turning steering wheel and is entirely hand-finished.',	5992,	'S18_3136',	10472,	0,	7),
(39,	'1903 Ford Model A',	'Features opening trunk,  working steering system',	3913,	'S18_3140',	13659,	0,	7),
(40,	'1992 Ferrari 360 Spider red',	'his replica features opening doors, superb detail and craftsmanship, working steering system, opening forward compartment, opening rear trunk with removable spare, 4 wheel independent spring suspension as well as factory baked enamel finish.',	8347,	'S18_3232',	16934,	0,	1),
(41,	'1985 Toyota Supra',	'This model features soft rubber tires, working steering, rubber mud guards, authentic Ford logos, detailed undercarriage, opening doors and hood, removable split rear gate, full size spare mounted in bed, detailed interior with opening glove box',	7733,	'S18_3233',	10757,	0,	1),
(42,	'Collectable Wooden Train',	'Hand crafted wooden toy train set is in about 1:18 scale, 25 inches in total length including 2 additional carts, of actual vintage train. This antique style wooden toy train model set is all hand-assembled with 100% wood.',	6450,	'S18_3259',	10084,	0,	5),
(43,	'1969 Dodge Super Bee',	'This replica features opening doors, superb detail and craftsmanship, working steering system, opening forward compartment, opening rear trunk with removable spare, 4 wheel independent spring suspension as well as factory baked enamel finish.',	1917,	'S18_3278',	8041,	0,	1),
(44,	'1917 Maxwell Touring Car',	'Features Gold Trim, Full Size Spare Tire, Chrome Trim, Chrome Grille, Opening Hood, Opening Doors, Opening Trunk, Detailed Engine, Working Steering System',	7913,	'S18_3320',	9921,	0,	7),
(45,	'1976 Ford Gran Torino',	'Highly detailed 1976 Ford Gran Torino \"Starsky and Hutch\" diecast model. Very well constructed and painted in red and white patterns.',	9127,	'S18_3482',	14699,	0,	1),
(46,	'1948 Porsche Type 356 Roadster',	'This model features working front and rear suspension on accurately replicated and actuating shock absorbers as well as opening engine cover, rear stabilizer flap,  and 4 opening doors.',	8990,	'S18_3685',	14128,	0,	1),
(47,	'1957 Vespa GS150',	'Features rotating wheels , working kick stand. Comes with stand.',	7689,	'S18_3782',	6217,	0,	2),
(48,	'1941 Chevrolet Special Deluxe Cabriolet',	'Features opening hood, opening doors, opening trunk, wide white wall tires, front door arm rests, working steering system, leather upholstery. Color black.',	2378,	'S18_3856',	10587,	0,	7),
(49,	'1970 Triumph Spitfire',	'Features include opening and closing doors. Color: White.',	5545,	'S18_4027',	14362,	0,	1),
(50,	'1932 Alfa Romeo 8C2300 Spider Sport',	'This 1:18 scale precision die cast replica features the 6 front headlights of the original, plus a detailed version of the 142 horsepower straight 8 engine, dual spares and their famous comprehensive dashboard. Color black.',	6553,	'S18_4409',	9203,	0,	7),
(51,	'1904 Buick Runabout',	'Features opening trunk,  working steering system',	8290,	'S18_4522',	8777,	0,	7),
...
(53,	'1939 Cadillac Limousine',	'Features completely detailed interior including Velvet flocked drapes,deluxe wood grain floor, and a wood grain casket with seperate chrome handles',	6645,	'S18_4668',	5031,	0,	7),
(54,	'1957 Corvette Convertible',	'1957 die cast Corvette Convertible in Roman Red with white sides and whitewall tires. 1:18 scale quality die-cast with detailed engine and underbvody. Now you can own The Classic Corvette.',	1249,	'S18_4721',	14880,	0,	1),
(55,	'1957 Ford Thunderbird',	'This 1:18 scale precision die-cast replica, with its optional porthole hardtop and factory baked-enamel Thunderbird Bronze finish, is a 100% accurate rendition of this American classic.',	3209,	'S18_4933',	7127,	0,	1),
(56,	'1970 Chevy Chevelle SS 454',	'This model features rotating wheels, working streering system and opening doors. All parts are particularly delicate due to their precise scale and require special care and attention. It should not be picked up by the doors, roof, hood or trunk.',	1005,	'S24_1046',	7349,	0,	1),
(57,	'1970 Dodge Coronet',	'1:24 scale die-cast about 18\" long doors open, hood opens and rubber wheels',	4074,	'S24_1444',	5780,	0,	1),
(58,	'1997 BMW R 1100 S',	'Detailed scale replica with working suspension and constructed from over 70 parts',	7003,	'S24_1578',	11270,	0,	2),
(59,	'1966 Shelby Cobra 427 S/C',	'This diecast model of the 1966 Shelby Cobra 427 S/C includes many authentic details and operating parts. The 1:24 scale model of this iconic lighweight sports car from the 1960s comes in silver and it\'s own display case.',	8197,	'S24_1628',	5031,	0,	1),
(60,	'1928 British Royal Navy Airplane',	'Official logos and insignias',	3627,	'S24_1785',	10942,	0,	3),
(61,	'1939 Chevrolet Deluxe Coupe',	'This 1:24 scale die-cast replica of the 1939 Chevrolet Deluxe Coupe has the same classy look as the original. Features opening trunk, hood and doors and a showroom quality baked enamel finish.',	7332,	'S24_1937',	3319,	0,	7),
(62,	'1960 BSA Gold Star DBD34',	'Detailed scale replica with working suspension and constructed from over 70 parts',	15,	'S24_2000',	7617,	0,	2),
(64,	'1938 Cadillac V-16 Presidential Limousine',	'This 1:24 scale precision die cast replica of the 1938 Cadillac V-16 Presidential Limousine has all the details of the original, from the flags on the front to an opening back seat compartment complete with telephone and rifle. Features factory baked-enamel black finish, hood goddess ornament, working jump seats.',	2847,	'S24_2022',	4480,	0,	7),
(65,	'1962 Volkswagen Microbus',	'This 1:18 scale die cast replica of the 1962 Microbus is loaded with features: A working steering system, opening front doors and tailgate, and famous two-tone factory baked enamel finish, are all topped of by the sliding, real fabric, sunroof.',	2327,	'S24_2300',	12779,	0,	6),
(66,	'1982 Ducati 900 Monster',	'Features two-tone paint with chrome accents, superior die-cast detail , rotating wheels , working kick stand',	6840,	'S24_2360',	6926,	0,	2),
(67,	'1949 Jaguar XK 120',	'Precision-engineered from original Jaguar specification in perfect scale ratio. Features opening doors, superb detail and craftsmanship, working steering system, opening forward compartment, opening rear trunk with removable spare, 4 wheel independent spring suspension as well as factory baked enamel finish.',	2350,	'S24_2766',	9087,	0,	1),
(68,	'1958 Chevy Corvette Limited Edition',	'The operating parts of this 1958 Chevy Corvette Limited Edition are particularly delicate due to their precise scale and require special care and attention. Features rotating wheels, working streering, opening doors and trunk. Color dark green.',	2542,	'S24_2840',	3536,	0,	1),
(69,	'1900s Vintage Bi-Plane',	'Hand crafted diecast-like metal bi-plane is re-created in about 1:24 scale of antique pioneer airplane. All hand-assembled with many different parts. Hand-painted in classic yellow and features correct markings of original airplane.',	5942,	'S24_2841',	6851,	0,	3),
(70,	'1952 Citroen-15CV',	'Precision crafted hand-assembled 1:18 scale reproduction of the 1952 15CV, with its independent spring suspension, working steering system, opening doors and hood, detailed engine and instrument panel, all topped of with a factory fresh baked enamel finish.',	1452,	'S24_2887',	11744,	0,	1),
(71,	'1982 Lamborghini Diablo',	'This replica features opening doors, superb detail and craftsmanship, working steering system, opening forward compartment, opening rear trunk with removable spare, 4 wheel independent spring suspension as well as factory baked enamel finish.',	7723,	'S24_2972',	3776,	0,	1),
(72,	'1912 Ford Model T Delivery Wagon',	'This model features chrome trim and grille, opening hood, opening doors, opening trunk, detailed engine, working steering system. Color white.',	9173,	'S24_3151',	8851,	0,	7),
(73,	'1969 Chevrolet Camaro Z28',	'1969 Z/28 Chevy Camaro 1:24 scale replica. The operating parts of this limited edition 1:24 scale diecast model car 1969 Chevy Camaro Z28- hood, trunk, wheels, streering, suspension and doors- are particularly delicate due to their precise scale and require special care and attention.',	4695,	'S24_3191',	8561,	0,	1),
(74,	'1971 Alpine Renault 1600s',	'This 1971 Alpine Renault 1600s replica Features opening doors, superb detail and craftsmanship, working steering system, opening forward compartment, opening rear trunk with removable spare, 4 wheel independent spring suspension as well as factory baked enamel finish.',	7995,	'S24_3371',	6123,	0,	1),
(75,	'1937 Horch 930V Limousine',	'Features opening hood, opening doors, opening trunk, wide white wall tires, front door arm rests, working steering system',	2902,	'S24_3420',	6575,	0,	7),
(76,	'2002 Chevy Corvette',	'The operating parts of this limited edition Diecast 2002 Chevy Corvette 50th Anniversary Pace car Limited Edition are particularly delicate due to their precise scale and require special care and attention. Features rotating wheels, poseable streering, opening doors and trunk.',	9446,	'S24_3432',	10708,	0,	1),
(77,	'1940 Ford Delivery Sedan',	'Chrome Trim, Chrome Grille, Opening Hood, Opening Doors, Opening Trunk, Detailed Engine, Working Steering System. Color black.',	6621,	'S24_3816',	8386,	0,	7),
(78,	'1956 Porsche 356A Coupe',	'Features include: Turnable front wheels; steering function; detailed interior; detailed engine; opening hood; opening trunk; opening doors; and detailed chassis.',	6600,	'S24_3856',	14043,	0,	1),
(79,	'Corsair F4U ( Bird Cage)',	'Has retractable wheels and comes with a stand. Official logos and insignias.',	6812,	'S24_3949',	6824,	0,	3),
(80,	'1936 Mercedes Benz 500k Roadster',	'This model features grille-mounted chrome horn, lift-up louvered hood, fold-down rumble seat, working steering system and rubber wheels. Color black.',	2081,	'S24_3969',	4103,	0,	7),
(81,	'1992 Porsche Cayenne Turbo Silver',	'This replica features opening doors, superb detail and craftsmanship, working steering system, opening forward compartment, opening rear trunk with removable spare, 4 wheel independent spring suspension as well as factory baked enamel finish.',	6582,	'S24_4048',	11828,	0,	1),
(82,	'1936 Chrysler Airflow',	'Features opening trunk,  working steering system. Color dark green.',	4710,	'S24_4258',	9739,	0,	7),
(83,	'1900s Vintage Tri-Plane',	'Hand crafted diecast-like metal Triplane is Re-created in about 1:24 scale of antique pioneer airplane. This antique style metal triplane is all hand-assembled with many different parts.',	2756,	'S24_4278',	7245,	0,	3),
(84,	'1961 Chevrolet Impala',	'This 1:18 scale precision die-cast reproduction of the 1961 Chevrolet Impala has all the features-doors, hood and trunk that open; detailed 409 cubic-inch engine; chrome dashboard and stick shift, two-tone interior; working steering system; all topped of with a factory baked-enamel finish.',	7869,	'S24_4620',	8084,	0,	1),
(85,	'1980’s GM Manhattan Express',	'This 1980’s era new look Manhattan express is still active, running from the Bronx to mid-town Manhattan. Has 35 opeining windows and working lights. Needs a battery.',	5099,	'S32_1268',	9631,	0,	6),
(86,	'1997 BMW F650 ST',	'Features official die-struck logos and baked enamel finish. Comes with stand.',	178,	'S32_1374',	9989,	0,	2),
(87,	'1982 Ducati 996 R',	'Features rotating wheels , working kick stand. Comes with stand.',	9241,	'S32_2206',	4023,	0,	2),
(88,	'1954 Greyhound Scenicruiser',	'Model features bi-level seating, 50 windows, skylights & glare resistant glass, working steering system, original logos',	2874,	'S32_2509',	5411,	0,	6),
(89,	'1950\'s Chicago Surface Lines Streetcar',	'This streetcar is a joy to see. It has 80 separate windows, electric wire guides, detailed interiors with seats, poles and drivers controls, rolling and turning wheel assemblies, plus authentic factory baked-enamel finishes (Green Hornet for Chicago and Cream and Crimson for Boston).',	8601,	'S32_3207',	6214,	0,	5),
...
(91,	'1928 Ford Phaeton Deluxe',	'This model features grille-mounted chrome horn, lift-up louvered hood, fold-down rumble seat, working steering system',	136,	'S32_4289',	6879,	0,	7),
(92,	'1974 Ducati 350 Mk3 Desmo',	'This model features two-tone paint with chrome accents, superior die-cast detail , rotating wheels , working kick stand',	3341,	'S32_4485',	10205,	0,	2),
(93,	'1930 Buick Marquette Phaeton',	'Features opening trunk,  working steering system',	7062,	'S50_1341',	4364,	0,	7),
(94,	'Diamond T620 Semi-Skirted Tanker',	'This limited edition model is licensed and perfectly scaled for Lionel Trains. The Diamond T620 has been produced in solid precision diecast and painted with a fire baked enamel finish. It comes with a removable tanker and is a perfect model to add authenticity to your static train or car layout or to just have on display.',	1016,	'S50_1392',	11575,	0,	6),
(95,	'1962 City of Detroit Streetcar',	'This streetcar is a joy to see. It has 99 separate windows, electric wire guides, detailed interiors with seats, poles and drivers controls, rolling and turning wheel assemblies, plus authentic factory baked-enamel finishes (Green Hornet for Chicago and Cream and Crimson for Boston).',	1645,	'S50_1514',	5858,	0,	5),
(96,	'2002 Yamaha YZR M1',	'Features rotating wheels , working kick stand. Comes with stand.',	600,	'S50_4713',	8136,	0,	2),
(97,	'The Schooner Bluenose',	'All wood with canvas sails. Measures 31 1/2 inches in Length, 22 inches High and 4 3/4 inches Wide. Many extras.\r\nThe schooner Bluenose was built in Nova Scotia in 1921 to fish the rough waters off the coast of Newfoundland. Because of the Bluenose racing prowess she became the pride of all Canadians. Still featured on stamps and the Canadian dime, the Bluenose was lost off Haiti in 1946.',	1897,	'S700_1138',	6667,	0,	4),
(98,	'American Airlines: B767-300',	'Exact replia with official logos and insignias and retractable wheels',	5841,	'S700_1691',	9134,	0,	3),
(99,	'The Mayflower',	'Measures 31 1/2 inches Long x 25 1/2 inches High x 10 5/8 inches Wide\r\nAll wood with canvas sail. Extras include long boats, rigging, ladders, railing, anchors, side cannons, hand painted, etc.',	737,	'S700_1938',	8661,	0,	4),
(100,	'HMS Bounty',	'Measures 30 inches Long x 27 1/2 inches High x 4 3/4 inches Wide. \r\nMany extras including rigging, long boats, pilot house, anchors, etc. Comes with three masts, all square-rigged.',	3501,	'S700_2047',	9052,	0,	4),
(101,	'America West Airlines B757-200',	'Official logos and insignias. Working steering system. Rotating jet engines',	9653,	'S700_2466',	9972,	0,	3),
(102,	'The USS Constitution Ship',	'All wood with canvas sails. Measures 31 1/2\" Length x 22 3/8\" High x 8 1/4\" Width. Extras include 4 boats on deck, sea sprite on bow, anchors, copper railing, pilot houses, etc.',	7083,	'S700_2610',	7228,	0,	4),
(103,	'1982 Camaro Z28',	'Features include opening and closing doors. Color: White. \r\nMeasures approximately 9 1/2\" Long.',	6934,	'S700_2824',	10115,	0,	1),
(104,	'ATA: B757-300',	'Exact replia with official logos and insignias and retractable wheels',	7106,	'S700_2834',	11865,	0,	3),
(105,	'F/A 18 Hornet 1/72',	'10\" Wingspan with retractable landing gears.Comes with pilot',	551,	'S700_3167',	8000,	0,	3),
(106,	'The Titanic',	'Completed model measures 19 1/2 inches long, 9 inches high, 3inches wide and is in barn red/black. All wood and metal.',	1956,	'S700_3505',	10017,	0,	4),
(107,	'The Queen Mary',	'Exact replica. Wood and Metal. Many extras including rigging, long boats, pilot house, anchors, etc. Comes with three masts, all square-rigged.',	5088,	'S700_3962',	9931,	0,	4),
(108,	'American Airlines: MD-11S',	'Polished finish. Exact replia with official logos and insignias and retractable wheels',	8820,	'S700_4002',	7403,	0,	3),
(109,	'Boeing X-32A JSF',	'10\" Wingspan with retractable landing gears.Comes with pilot',	4857,	'S72_1253',	4966,	0,	3),
(110,	'Pont Yacht',	'Measures 38 inches Long x 33 3/4 inches High. Includes a stand.\r\nMany extras including rigging, long boats, pilot house, anchors, etc. Comes with 2 masts, all square-rigged',	414,	'S72_3212',	5460,	0,	4),
(128,	'Harley pack',	'',	0,	'',	34000,	0,	2),
(129,	'Mustangs',	'',	0,	'',	27000,	0,	7),
(130,	'1969',	'',	0,	'',	66000,	0,	6),
(136,	'test pack',	NULL,	0,	NULL,	40000,	0,	1),
(141,	'Test',	NULL,	0,	NULL,	13300,	0,	7),
(142,	'PC portable',	NULL,	0,	NULL,	79900,	0,	27),
(143,	'Ecran',	NULL,	0,	NULL,	2200,	0,	27),
(145,	'Autre',	NULL,	0,	NULL,	2500,	0,	27),
(147,	'Prod Romero',	NULL,	0,	NULL,	2200,	0,	41);

DELIMITER ;;

CREATE TRIGGER `update_product_price` BEFORE UPDATE ON `product` FOR EACH ROW
BEGIN
...
CREATE TABLE `purchase` (
  ...
  `toPay` mediumint DEFAULT NULL,
  `itemsNumber` int NOT NULL,
  `missingNumber` int NOT NULL,
  `idTimeslot` int DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idUser` (`idUser`),
  KEY `idEmployee` (`idEmployee`),
  KEY `idTimeslot` (`idTimeslot`),
  CONSTRAINT `purchase_ibfk_1` FOREIGN KEY (`idUser`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `purchase_ibfk_2` FOREIGN KEY (`idEmployee`) REFERENCES `employee` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `purchase_ibfk_3` FOREIGN KEY (`idTimeslot`) REFERENCES `timeslot` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=55 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

TRUNCATE `purchase`;
INSERT INTO `purchase` (`id`, `dateCreation`, `idUser`, `idEmployee`, `status`, `amount`, `toPay`, `itemsNumber`, `missingNumber`, `idTimeslot`) VALUES
(1,	'2021-03-03 12:10:31',	1,	1,	0,	9600,	200,	1,	1,	1),
(3,	'2021-03-03 18:44:16',	1,	2,	1,	14774,	200,	1,	1,	1),
(4,	'2021-03-04 10:52:53',	1,	1,	0,	9921,	200,	1,	1,	2),
(8,	'2021-03-04 11:05:50',	1,	2,	0,	27470,	200,	3,	3,	2),
(9,	'2021-03-05 11:43:03',	2,	3,	0,	11894,	200,	1,	1,	2),
(13,	'2021-03-06 13:12:42',	3,	1,	1,	97285,	0,	5,	5,	3),
(15,	'2021-03-07 11:56:07',	3,	2,	0,	173020,	200,	10,	10,	3),
(19,	'2021-04-11 13:18:50',	1,	NULL,	0,	999999,	200,	195,	195,	NULL),
(24,	'2021-04-11 13:32:40',	1,	NULL,	0,	801810,	200,	70,	70,	NULL),
(25,	'2021-04-25 15:39:31',	1,	1,	0,	14650,	200,	2,	2,	6101),
(26,	'2021-04-26 17:28:59',	1,	NULL,	0,	0,	200,	0,	0,	NULL),
(27,	'2021-04-26 17:32:49',	1,	NULL,	0,	19457,	200,	1,	1,	NULL),
(28,	'2021-04-28 10:01:37',	1,	NULL,	0,	27000,	200,	1,	1,	NULL),
(51,	'2022-05-21 19:24:16',	1,	1,	0,	99520,	NULL,	8,	8,	6539),
(53,	'2022-05-21 20:01:58',	1,	1,	0,	187108,	NULL,	14,	14,	6442),
(54,	'2022-05-21 20:02:22',	1,	3,	1,	1477400,	NULL,	100,	100,	6440);

DELIMITER ;;

CREATE TRIGGER `before_insert_order` BEFORE INSERT ON `purchase` FOR EACH ROW
BEGIN
...
CREATE TRIGGER `after_insert_order` AFTER INSERT ON `purchase` FOR EACH ROW
if (NEW.idTimeslot is NOT NULL) THEN
    call updateTimeSlot(NEW.idTimeslot);
END IF;;

CREATE TRIGGER `update_order` AFTER UPDATE ON `purchase` FOR EACH ROW
if (NEW.idTimeslot is NOT NULL) THEN
    call updateTimeSlot(NEW.idTimeslot);
...
CREATE TRIGGER `delete_order` AFTER DELETE ON `purchase` FOR EACH ROW
if (OLD.idTimeslot is NOT NULL) THEN
    call updateTimeSlot(OLD.idTimeslot);
end if;;
...
CREATE TABLE `purchasedetail` (
  ...
  KEY `idProduct` (`idProduct`),
  CONSTRAINT `purchasedetail_ibfk_1` FOREIGN KEY (`idOrder`) REFERENCES `purchase` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `purchasedetail_ibfk_2` FOREIGN KEY (`idProduct`) REFERENCES `product` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

TRUNCATE `purchasedetail`;
INSERT INTO `purchasedetail` (`idOrder`, `idProduct`, `quantity`, `prepared`) VALUES
(3,	6,	1,	0),
(4,	44,	1,	0),
(8,	27,	1,	0),
(8,	76,	2,	0),
...
CREATE TRIGGER `insert_order_detail` AFTER INSERT ON `purchasedetail` FOR EACH ROW
CALL updateOrderAmount (
        NEW.idOrder
    );;

CREATE TRIGGER `update_order_detail` AFTER UPDATE ON `purchasedetail` FOR EACH ROW
CALL updateOrderAmount (
        NEW.idOrder
    );;

CREATE TRIGGER `delete_order_detail` AFTER DELETE ON `purchasedetail` FOR EACH ROW
CALL updateOrderAmount (
        OLD.idOrder
    );;
...
(1,	'09:00:00',	'[0, 1, 2, 3, 4]'),
(2,	'09:15:00',	'[0, 1, 2, 3, 4]'),
(3,	'09:30:00',	'[0, 1, 2, 3, 4]'),
(4,	'09:45:00',	'[0, 1, 2, 3, 4]'),
(5,	'10:00:00',	'[0, 1, 2, 3, 4]'),
(6,	'10:15:00',	'[0, 1, 2, 3, 4]'),
(7,	'10:30:00',	'[0, 1, 2, 3, 4]'),
(8,	'10:45:00',	'[0, 1, 2, 3, 4]'),
(9,	'11:00:00',	'[0, 1, 2, 3, 4]'),
(10,	'11:15:00',	'[0, 1, 2, 3, 4]'),
(11,	'11:30:00',	'[0, 1, 2, 3, 4]'),
(12,	'11:45:00',	'[0, 1, 2, 3, 4]'),
(13,	'12:00:00',	'[0, 1, 2, 3, 4]'),
(14,	'14:00:00',	'[0, 1, 2, 3, 4]'),
(15,	'14:15:00',	'[0, 1, 2, 3, 4]'),
(16,	'14:30:00',	'[0, 1, 2, 3, 4]'),
(17,	'14:45:00',	'[0, 1, 2, 3, 4]'),
(18,	'15:00:00',	'[0, 1, 2, 3, 4]'),
(19,	'15:15:00',	'[0, 1, 2, 3, 4]'),
(20,	'15:30:00',	'[0, 1, 2, 3, 4]'),
(21,	'15:45:00',	'[0, 1, 2, 3, 4]'),
(22,	'16:00:00',	'[0, 1, 2, 3, 4]'),
(23,	'16:15:00',	'[0, 1, 2, 3, 4]'),
(24,	'16:30:00',	'[0, 1, 2, 3, 4]'),
(25,	'16:45:00',	'[0, 1, 2, 3, 4]'),
(26,	'17:00:00',	'[0, 1, 2, 3, 4]'),
(27,	'17:15:00',	'[0, 1, 2, 3, 4]'),
(28,	'17:30:00',	'[0, 1, 2, 3, 4]'),
(29,	'17:45:00',	'[0, 1, 2, 3, 4]');
...
CREATE TABLE `timeslot` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `slotDate` (`slotDate`)
) ENGINE=InnoDB AUTO_INCREMENT=32260 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

TRUNCATE `timeslot`;
INSERT INTO `timeslot` (`id`, `slotDate`, `full`, `expired`) VALUES
(1,	'2021-03-03 12:00:00',	0,	1),
(2,	'2021-03-04 12:00:00',	1,	1),
...
(6440,	'2021-05-12 15:30:00',	0,	1),
(6441,	'2021-05-12 15:45:00',	0,	0),
(6442,	'2021-05-12 16:00:00',	0,	1),
...
CREATE TRIGGER `insert_user_basket` AFTER INSERT ON `user` FOR EACH ROW
BEGIN
